...
const productItemElement = async (value) => {
    let productChoise = `https://api.mercadolibre.com/sites/MLB/search?q=computador`;
    if (value) productChoise = `https://api.mercadolibre.com/sites/MLB/search?q=${value}`;
  try {
    const items = document.querySelector('.items');
...
const getInputValue = () => {
  ...
  productItemElement(input.value);
}
